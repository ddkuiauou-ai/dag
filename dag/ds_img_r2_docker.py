...
@dg.asset(
    ...
    description="DS 프로젝트 - 도커 컨테이너 실시간 로그 스트리밍으로 이미지 처리"
)
def ds_img_r2_docker(
    context: dg.AssetExecutionContext
) -> dg.MaterializeResult:
    """DS 이미지 처리용 도커 컨테이너를 실시간 로그 스트리밍으로 실행합니다."""
    ...
    try:
        context.log.info("🚀 DS 이미지 처리 컨테이너 시작")
        
        # DS 이미지 처리용 도커 컨테이너 실행
        container = docker_client.containers.run(
            image="ds-naver-crawler:dev",
            detach=True,
            stdout=True,
            stderr=True,
            remove=False,
        ...
        context.log.info(f"📦 DS 컨테이너 시작됨: {container.short_id}")
                ...
                if log_text:
                    ...
                    if "Company logo saved:" in clean_log:
                        ...
                        if filename_match:
                            ...
                            context.log.info(f"✅ DS 로고 저장 완료: {filename}")
                        else:
                            context.log.info(f"📸 DS 로고 처리: {clean_log}")
                    ...
                    # 크롤링 완료 통계 추출
                        ...
                        context.log.info(f"🏁 DS 크롤링 완료: {clean_log}")
                    
                    else:
                        # 일반 로그 출력 (깔끔하게 정리됨)
                        context.log.info(f"📋 DS_PROCESSOR: {clean_log}")
                    
                    log_count += 1
                    
                    # DS 이미지 처리는 상대적으로 로그가 적을 것으로 예상
                    # if log_count > 1500:
        ...
        context.log.info(f"=== ✅ DS 이미지 처리 완료 (exit_code: {exit_code['StatusCode']}) ===")
        ...
        if final_logs.strip():
            context.log.info("📝 DS 처리 최종 로그:")
            for line in final_logs.split('\n')[-5:]:
                if line.strip():
                    clean_final = clean_ansi_codes(line)
                    context.log.info(f"🔚 DS_FINAL: {clean_final}")
        ...
        context.log.error(f"❌ DS 이미지 처리 실행 실패: {e}")
        raise
    finally:
        # 컨테이너 정리
        if container:
            try:
                container.stop()
                container.remove()
                context.log.info("🧹 DS 컨테이너 정리 완료")
            except Exception as e:
                context.log.warning(f"DS 컨테이너 정리 중 오류: {e}")
