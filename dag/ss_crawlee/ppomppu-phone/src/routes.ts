...
/****
 * ---------------------------------------------------------------------------
 * PPOMPPU Phone Forum Router (std-optimized)
 *
 * 역할
...
export const BOARDS: Record<string, BoardConfig> = {
    phone: makePpomppuPhoneBoard({
        siteName: 'ppomppu',
        boardName: 'phone',
        baseUrl: 'https://www.ppomppu.co.kr',
        baseListUrl: 'https://www.ppomppu.co.kr/zboard/zboard.php?id=phone',
    }),
};
...
function makePpomppuBoard(opts: BoardOptions): BoardConfig {
    ...
    return {
        ...
        list: {
            ...
            timestampHint: 'time.baseList-time',
...
function makePpomppuPhoneBoard(opts: BoardOptions): BoardConfig {
    const STORE_KEY = `CRAWL_STATE_${opts.siteName.toUpperCase()}_${opts.boardName.replace(/[^a-z0-9]/gi, '_').toUpperCase()}`;
    // 휴대폰포럼(zboard.php?id=phone) 전용 셀렉터/규칙
    return {
        SITE_NAME: opts.siteName,
        BOARD_NAME: opts.boardName,
        STATE_STORE_NAME: STORE_KEY,
        baseListUrl: opts.baseListUrl,
        // 상세 URL 허용: 같은 호스트, /zboard/view.php 또는 /zboard/zboard.php, id=phone, no=숫자
        postUrlAllow: (u, base) => {
            if (u.hostname !== base.hostname) return false;
            if (!(u.pathname === '/zboard/view.php' || u.pathname === '/zboard/zboard.php')) return false;
            const id = u.searchParams.get('id');
            const no = u.searchParams.get('no');
            return id === 'phone' && !!(no && /^\d+$/.test(no));
        },
        list: {
            // 리스트 테이블은 skin/DQ_Revolution_BBS 기반으로 #revolution_main_table 을 사용
            // tbody 유무가 페이지마다 달라질 수 있으므로 테이블 전체를 컨테이너로 지정
            container: '#revolution_main_table',
            // 게시물 행: board list row class로 매칭 (공지/광고/스폰서 등은 제외)
            item: 'tr.baseList',
            // 상세 링크 앵커: baseList-title 전용
            postLink: 'a.baseList-title',
            // 목록 시간 힌트(옵션): 등록일 컬럼은 보통 영문 숫자(eng) 스타일을 사용
            // 날짜/시간이 앞쪽에 먼저 나오는 td.list_vspace도 우선 포함하여 범위를 넓힘
            timestampHint: 'td.list_vspace, td:has(nobr), td.eng, td:has(span.eng)',
            // 공지/홍보 스킵
            // 공지 행이 목록에 섞여도 robust하게 제외
            skipNotice: 'tr.notice, tr[class*="notice"], td:has(img[src*="icon_notice"])',
            skipPromo: ':has(#ad-icon), :has([id*="ad-icon"]), :has([class*="ad-icon"]), :has([aria-label*="광고"]), :has([data-ad]), :has([data-sponsored]), :has(img.baseList-img[title*="비밀글"]), :has(img[title*="비밀글"]), :has(img[src*="icon_01a"])',
            pageParam: 'page',
            nextPageSelectors: [
                'a[rel="next"]',
                'a.next_page',
        ...
        detail: {
            // 본문/제목/작성자 영역 (skin 공통 구조)
            content: 'td.board-contents',
            title: '#topTitle h1',
            // 작성자명은 .topTitle-name 내부 a.baseList-name 에 존재
            author: '#topTitle .topTitle-name .baseList-name, #topTitle .topTitle-name a',
            // 메타 블록(작성일/조회수 등)
            metaBlock: '#topTitle .topTitle-mainbox',
            // 아바타: 휴대폰포럼은 background-image 가 적용된 .topTitle-profile 을 우선 사용
            authorAvatarSelectors: [
                '#topTitle .topTitle-profile',
                '#topTitle .topTitle-profile img',
                'img[src*="PPOM_icon/"]'
            ...
            // 작성자 옆 형제 li 에 작성일 텍스트가 위치
            timestampIsSiblingOfAuthor: true,
            // 대표 작성일 셀 (예: "작성일 2025-09-05 14:33")
            timestampSelectors: [
                '#topTitle .topTitle-mainbox li:nth-of-type(2)'
            ],
            // 조회수/추천 수 텍스트 셀(있으면 추출, 없으면 생략)
            viewCount: '#topTitle .topTitle-mainbox li:has-text("조회")',
            likeCount: '#topTitle .topTitle-mainbox li:has-text("추천")',
            dislikeCount: '#topTitle .topTitle-mainbox li:has-text("비추천")',
            ...
            // 본문 내 자원/임베드
            tagSelectors: [],
            imageSelectors: ['td.board-contents img'],
            iframeSelectors: ['td.board-contents iframe[src]'],
            twitterSelectors: ['blockquote.twitter-tweet'],
            restrictedTextProbes: ['td.board-contents'],
            restrictedTextRegex: [/권한이 없|삭제된|존재하지 않는/],
        },
    };
}
...
export function createRouter(cfg: BoardConfig, outDatasetId = 'posts') {
    ...
    router.addHandler("LIST", async ({ page, enqueueLinks, log, request }) => {
        ...
        for (const postElement of postElements) {
            ...
            // Full 모드 중단 조건 (timestampHint → 실패 시 행 전체 스캔으로 보강)
            if (mode === 'full' && cutoffDate) {
                // 1) 우선 cfg.list.timestampHint로 시도
                let tsText = '';
                if (cfg.list.timestampHint) {
                    try {
                        tsText = await postElement.evaluate((el, hintSel) => {
                            const scope = el as HTMLElement;
                            const sel = hintSel as string;
                            // time 엘리먼트가 최우선
                            const timeEl = scope.querySelector('time.baseList-time') || scope.querySelector(sel);
                            if (!timeEl) return '';
                            // 부모 TD 또는 자신에게 title이 있으면 그것을 최우선으로 사용 (예: "25.09.14 00:11:14")
                            const td = timeEl.closest('td');
                            const title = (td && td.getAttribute('title')) || (timeEl as HTMLElement).getAttribute?.('title') || '';
                ...
                // 2) 실패/부적합(닉네임 등) 시: 행의 모든 TD에서 날짜/시간 패턴 후보를 탐색
                let trimmed = (tsText || '').replace('등록', '').trim();
                const looksLikeName = trimmed && !/[0-9]/.test(trimmed); // 숫자가 전혀 없으면 시간일 확률 낮음
                if (!trimmed || looksLikeName) {
                    try {
                        const candidate = await postElement.evaluate((el) => {
                            const getText = (n: Element) => (n.textContent || '').replace(/\s+/g, ' ').trim();
                            const tds = Array.from(el.querySelectorAll('td'));
                            const texts = tds.map(getText).filter(Boolean);
                            // 그럴듯한 시간/날짜 패턴 (우선순위)
                            const patterns = [
                                /\d{4}[./-]\d{2}[./-]\d{2}\s+\d{2}:\d{2}(?::\d{2})?/, // YYYY.MM.DD HH:mm[:ss]
                                /\d{2}[./-]\d{2}[./-]\d{2}\s+\d{2}:\d{2}(?::\d{2})?/, // YY.MM.DD HH:mm[:ss]
                ...
                if (trimmed) {
                    try {
                        ...
                        try { log.debug?.(`[timestamp][LIST] parsed ok: postId=${postId} ts="${postTimestamp}" from hint/candidate="${trimmed}" url=${page.url()}`); } catch { /* ignore */ }
                    } catch (err) {
                        log.warning?.(`[timestamp][LIST] parse failed for postId=${postId} hint/candidate="${trimmed}" url=${page.url()} err=${String(err)}`);
                    }
                }
