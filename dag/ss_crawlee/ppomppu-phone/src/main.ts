import { PlaywrightCrawler, log, Dataset, KeyValueStore, PlaywrightGotoOptions, Configuration } from "crawlee";
...
/**
 * ---------------------------------------------------------------------------
 * PPOMPPU Hot Crawler Entry (std-optimized)
 *
 * 역할
 *  • 실행 진입점. 실제 셀렉터/규칙은 `routes.ts`에서 정의하고,
 *    여기서는 **크롤 모드/CLI 파싱/실행/결과 저장 및 검증**을 담당합니다.
 *
 * 모드
...
// ===[ 공통 상수 ]===========================================================
const BLOCKED_RESOURCE_TYPES = ["stylesheet", "font", "media", "image"] as const;
...
// ===[ 유틸 ]=================================================================
/**
 * 문자열을 Date로 파싱 가능하면 ISO 문자열로, 아니면 원문을 그대로 반환합니다.
 * - 결과 JSON의 기존 동작(파싱 불가 시 원문 유지)을 보존합니다.
 */
...
/**
 * 중첩된 댓글 트리를 깊이 우선 순회하여 **평면 개수**로 카운트합니다.
 * - 각 댓글 노드 1개당 1로 계산합니다.
 */
...
/**
 * 결과 아이템을 표준 형태로 정규화하고 간단한 유효성 검사를 수행합니다.
 * 보존되는 규칙
 *  - `id`/`post_id`는 문자열화 + 중복 여부 수집(출력 형식 불변)
 *  - `image_count`/`embed_count`/`comment_count` 순으로 정규화
 *  - 시간 필드는 파싱 가능 시에만 ISO로 변환
 */
...
for (const bname of selectedBoards) {
        ...
        case 'hot': {
            ...
            startUrls = hotUrls.map((url: string) => {
                ...
                try {
                    ...
                    log.warning(`Hot 모드에서 잘못된 URL 발견: ${url}`, e);
                }
    ...
    // ===[ 결과 처리 및 저장 (보드별) ]======================================
    {
        ...
        const backupDir = path.resolve(process.cwd(), '../../../ssdata');
...
if (process.env.DIAG === '1') {
    // 다음 틱에서 진단 출력 → top-level await 체인과 분리
    setImmediate(async () => {
        try {
    ...
    // DIAG 아니면 즉시 종료 보증 (stdout flush 후 종료하고 싶으면 setImmediate로 감싸도 됨)
    process.exit(0);
}