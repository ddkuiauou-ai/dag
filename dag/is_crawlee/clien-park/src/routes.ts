 ...
 *  - HTML 구조가 유사한 경우, CONFIG와 셀렉터만 교체하여 재사용할 수 있습니다.
 *
 * 유지보수 원칙 (⚠ 결과 JSON 불변)
 *  - 본 파일의 리팩토링은 **결과 JSON 스키마/값을 변경하지 않습니다.**
 *  - 최적화는 가독성과 견고성 위주로 하며, 동작은 동일하게 유지합니다.
 *  - 과도한 폴백/실험용(주석 처리된) 코드는 제거하되, 실제 수집에 필요할 수 있는 폴백은 보존합니다.
 *
 * 구성
 *  - 타입 정의: Comment / Embedded / BoardConfig
 *  - 유틸: URL/텍스트 정규화, 시간 파서(KST→UTC ISO), 임베드/이미지 수집
 *  - 라우터: LIST(목록 수집/페이징), DETAIL(본문/댓글/임베드 수집)
 * ---------------------------------------------------------------------------
 */
...
export type BoardConfig = {
    ...
    detail: {
        ...
        commentReactions?: string;      // 댓글 반응 수
...
export const BOARDS: Record<string, BoardConfig> = {
    park: makeBoard({
        siteName: 'clien',
        boardName: 'park',
        baseUrl: 'https://www.clien.net/service/board',
    }),
};

// 클리앙 모두의공원(board=park) 전용 BoardConfig 팩토리
function makeBoard(opts: BoardOptions): BoardConfig {
    ...
    return {
        ...
        // 클리앙 전용: board-agnostic allow rule
        postUrlAllow: (u, base, cfg) => {
            const basePath = new URL(cfg.baseListUrl).pathname; // e.g., /service/board/park
            return (u.hostname === base.hostname) && u.pathname.startsWith(basePath);
        },
        // LIST: 앵커 기반 추출(안정성 우선), 페이지네이션은 텍스트/onclick 기반 버튼 클릭
        list: {
            container: '.list_content',
            item: '.list_item[data-role="list-row"]',
            postLink: 'a.list_subject',
            timestampHint: '.list_time .timestamp',
            skipNotice: undefined,
            skipPromo: undefined,
            pageParam: 'po',
            nextPageSelectors: undefined,
        },
        // DETAIL: 제목/본문/시간 셀렉터 설정
        detail: {
            content: 'article',
            title: 'h3.post_subject',
            titleCategoryLink: undefined,
            // 작성자는 페이지 구조상 고정 셀렉터가 약해 후보 셀렉터 시도
            author: '.post_info .nickname :is(span,img)[title]',
            authorAvatarSelectors: [
                '.post_info .nickname img[title]'
            ],
            commentAvatarSelectors: [
                '.comment_info .nickname img[title]'
            ],
            timestampIsSiblingOfAuthor: false,
            timestampSelectors: ['span.view_count.date'],
            viewCount: 'div.post_author span.view_count strong',
            likeCount: 'a.post_symph span, a.symph_count strong',
            dislikeCount: undefined,
            // 댓글 (클리앙 모두의공원 구조)
            commentLikeCount: 'button[data-role="comment-like-count"] strong',
            commentDislikeCount: undefined,
            commentsContainer: 'div.comment[data-role="comment"], .comment[data-role="comment"]',
            commentArticle: '.comment_row[data-role="comment-row"]',
            commentAuthor: '.comment_info .nickname :is(span,img)[title]',
            commentTime: '.comment_time',
            commentReactions: undefined,
            commentContentHtml: '.comment_content .comment_view',
            commentRawTextareaPrefix: undefined,
            tagSelectors: undefined,
            // 리소스
            imageSelectors: ['article img'],
            iframeSelectors: ['article iframe[src]'],
            twitterSelectors: ['blockquote.twitter-tweet'],
            restrictedTextProbes: ['article'],
            restrictedTextRegex: [/권한이없|삭제|존재하지|비공개/],
        },
    };
}
...
export function createRouter(cfg: BoardConfig, outDatasetId = 'posts') {
    ...
    router.addHandler("LIST", async ({ page, enqueueLinks, log, request }) => {
        ...
        // Ensure TS __name helper exists in page context (current doc + future navigations)
        try {
            await page.addInitScript(() => {
                // @ts-ignore
                const w = (window as any);
                if (!w.__name) w.__name = (fn: any, _n: string) => fn;
            });
        } catch {}
        try {
            await page.evaluate(() => {
        ...
        // 미리 계산된 기준 URL 구성요소(호스트/경로): 상세 URL 허용 검사에서 재사용
        const baseUrlObj = new URL(cfg.baseListUrl);
        const baseHost = baseUrlObj.hostname;
        const basePath = baseUrlObj.pathname;

        await page.waitForSelector(`${cfg.list.container} ${cfg.list.item}`, { timeout: 5000 });
        ...
        const pageParam = cfg.list.pageParam || 'po';
        const pageUrl = new URL(page.url());
        const isFirstPage = !pageUrl.searchParams.has(pageParam) || pageUrl.searchParams.get(pageParam) === '0' || pageUrl.searchParams.get(pageParam) === '1';

        for (const postElement of postElements) {
            // 상세 링크
            const postUrl = await postElement.$(cfg.list.postLink).then(async (a) => a ? (await a.evaluate((el) => (el as HTMLAnchorElement).href)) : null);
            if (!postUrl) continue;

            // 허용 도메인/경로 확인
            try {
                const u = new URL(postUrl);
                const base = baseUrlObj;
                const allow = cfg.postUrlAllow
                    ? cfg.postUrlAllow(u, base, cfg)
                    : (u.hostname === baseHost && u.pathname.startsWith(basePath));
                if (!allow) continue;
            } catch { continue; }

            // postId: data-board-sn 우선, 실패 시 URL에서 추출
            let postId = await postElement.getAttribute('data-board-sn');
            if (!postId) {
                const m = postUrl.match(/\/service\/board\/[^/]+\/(\d+)(?=[^\d]|$)/);
                postId = m ? m[1] : null;
            }
            if (!postId) continue;
            ...
            // Full 모드 cutoff: 목록의 명시 timestamp 사용
            if (mode === 'full' && cutoffDate && cfg.list.timestampHint) {
                ...
                try {
                    ...
                    log.debug?.(`[timestamp][LIST] parsed ok: postId=${postId} ts="${postTimestamp}" from hint="${trimmed}" url=${page.url()}`);
                } catch (err) {
        ...
        // 다음 페이지 이동: Clien은 링크가 javascript: 형태라 href가 없음 → `po` 파라미터로 직접 구성
        if (!shouldStop) {
            const hardMax = (typeof maxPages === 'number' && isFinite(maxPages)) ? maxPages : null;
            if (hardMax !== null && pagesSoFar >= hardMax) {
                ...
                try {
                    // 현재 po는 URL 또는 #startPagePo(hidden)에서 확인
                    let curPo: number | null = null;
                    try {
                        const loc = page.locator('#startPagePo').first();
                        if (await loc.count()) {
                            const v = await loc.getAttribute('value');
                            if (v && /^\d+$/.test(v)) curPo = parseInt(v, 10);
                        }
                    } catch { /* ignore */ }

                    const u = new URL(page.url());
                    const fromUrl = u.searchParams.get('po');
                    if (fromUrl && /^\d+$/.test(fromUrl)) curPo = parseInt(fromUrl, 10);

                    const nextPo = (curPo === null || Number.isNaN(curPo)) ? 1 : (curPo + 1);
                    u.searchParams.set('po', String(nextPo));
                    // 등록일순 보장 (od가 없으면 기본값 지정)
                    if (!u.searchParams.get('od')) u.searchParams.set('od', 'T31');
                    nextUrl = u.toString();
                } catch { nextUrl = null; }
    ...
    router.addHandler("DETAIL", async ({ request, page, log }) => {
        ...
        // 제목은 h3.post_subject 안의 본문 <span>만 사용하고, 댓글 수(.post_reply)는 제거한다
        const fullTitle = await page.locator(cfg.detail.title).first().evaluate((el) => {
            const node = el.cloneNode(true) as HTMLElement;
            const reply = node.querySelector('.post_reply');
            if (reply) reply.remove();
            const span = node.querySelector('span');
            const text = (span?.textContent ?? node.textContent ?? '').replace(/\s+/g, ' ').trim();
            return text;
        });
        let title = (fullTitle || '').trim();
        if (category) title = title.replace(category, '').trim();
        ...
        // --- 작성자/시간 ---
        const authorLoc = page.locator(cfg.detail.author).first();
        let author: string | null = null;
        if (await authorLoc.count()) {
            const tagName = await authorLoc.evaluate((el) => (el as HTMLElement).tagName.toLowerCase());
            if (tagName === 'img') {
                author = (await authorLoc.getAttribute('title'))?.trim()
                    || (await authorLoc.getAttribute('alt'))?.trim()
                    || null;
            } else {
                author = (await authorLoc.textContent())?.replace(/\s+/g, ' ').trim()
                    || (await authorLoc.getAttribute('title'))?.trim()
                    || null;
            }
        }
            ...
            for (const sel of cfg.detail.timestampSelectors || []) {
                try {
                    ...
                    if (await loc.count()) {
                        ...
                        // Prefer textContent with the `.lastdate` child removed
                        let cleanedText: string | null = null;
                        try {
                            cleanedText = await loc.evaluate((el) => {
                                const clone = el.cloneNode(true) as HTMLElement;
                                const last = clone.querySelector('.lastdate');
                                if (last) last.remove();
                                return (clone.textContent || '').trim();
                            });
                        } catch { /* ignore */ }
                        const text = await loc.textContent();
                        rawTimestamp = (attrDatetime || attrTitle || cleanedText || text || '').trim();
        ...
        // Sanitize cases like: "2025-08-09 17:26:04\n수정일 : 2025-08-09 17:41:46"
        let sanitizedTs = (rawTimestamp || '').replace(/\s+/g, ' ').trim();
        // If both created/modified are present, take the first datetime occurrence
        const dtMatch = sanitizedTs.match(/\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}(?::\d{2})?/);
        if (dtMatch) sanitizedTs = dtMatch[0];
        // Final fallback: drop any trailing "수정일 : ..."
        sanitizedTs = sanitizedTs.replace(/수정일\s*:\s*\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}(?::\d{2})?/g, '').trim();

        const timestamp = parseKstToUtcIso(sanitizedTs || '', `DETAIL ${cfg.BOARD_NAME} ${request.loadedUrl} postId=${postId}`);
        ...
        // Post like/dislike: Clien shows either `.post_symph span` or `.symph_count strong`
        let like_count: number | null = null;
        try {
            if (cfg.detail.likeCount) {
                const likeLoc = page.locator(cfg.detail.likeCount).first();
                if (await likeLoc.count()) like_count = extractInt(await likeLoc.textContent());
            }
        } catch { like_count = null; }
        if (like_count === null) like_count = 0; // default 0 when absent

        const dislike_count: number = 0; // Clien detail does not expose a dislike; default to 0
        ...
        // Ensure twitter iframe rendered: conditionally nudge only if size is tiny (keep waits minimal)
        try {
        ...
        const collectComments = async (): Promise<{ topLevel: Comment[]; flatCount: number }> => {
            ...
            if (!cfg.detail.commentArticle) return { topLevel, flatCount: 0 };
            try {
                const raws = await page.$$eval(
                    cfg.detail.commentArticle,
                    (nodes, opts) => {
                        const selAuthor = (opts as any).commentAuthor as string | undefined;
                        const selAvatarArr = (opts as any).commentAvatarSelectors as string[] | undefined;
                        const selTime = (opts as any).commentTime as string | undefined;
                        const selHtml = (opts as any).commentContentHtml as string | undefined;
                        const selLike = (opts as any).commentLikeCount as string | undefined;
                        ...
                        for (const node of nodes as Element[]) {
                            ...
                            // CID
                            let cid = root.getAttribute('data-comment-sn') || '';
                            if (!cid) {
                                const point = root.querySelector('.comment_point');
                                cid = (point?.getAttribute('id') || '').trim();
                            }
                            if (!cid) continue;

                            // parentCid from onclick
                            let parentCid: string | null = null;
                            const btn = root.querySelector('.comment_menu button[onclick*="commentReMentionOpen("]') as HTMLButtonElement | null;
                            if (btn) {
                                const onclick = btn.getAttribute('onclick') || '';
                                const m = onclick.match(/commentReMentionOpen\('([0-9]+)'\s*,\s*'([0-9]+)'/);
                                if (m) parentCid = m[2];
                            }

                            // depth from class reN or margin-left
                            let depth = 0;
                            const klass = root.getAttribute('class') || '';
                            const rm = klass.match(/\bre(\d*)\b/);
                            if (rm) depth = rm[1] ? parseInt(rm[1], 10) : 1;
                            if (!rm) {
                                const ml = (root as HTMLElement).style.marginLeft || window.getComputedStyle(root).marginLeft || '';
                                const pxMatch = String(ml).match(/([\d.]+)px/);
                                if (pxMatch) {
                                    const px = parseFloat(pxMatch[1]);
                                    depth = Math.max(1, Math.round(px / 16));
                                } else {
                                    const remMatch = String(ml).match(/([\d.]+)rem/);
                                    if (remMatch) depth = Math.max(1, Math.round(parseFloat(remMatch[1])));
                                }
                            }
                            ...
                            let author: string | null = null;
                            if (selAuthor) {
                                const n = root.querySelector(selAuthor) as HTMLElement | null;
                                if (n) {
                                    const tag = n.tagName.toLowerCase();
                                    if (tag === 'img') {
                                        author = (n.getAttribute('title') || n.getAttribute('alt') || '').trim() || null;
                                    } else {
                                        author = (n.textContent || '').replace(/\s+/g, ' ').trim() || (n.getAttribute('title') || '').trim() || null;
                                    }
                                }
                            }
                            ...
                            // avatar
                            let avatar: string | null = null;
                            if (Array.isArray(selAvatarArr)) {
                                for (const s of selAvatarArr) {
                                    const img = root.querySelector(s) as HTMLImageElement | null;
                                    if (img?.src?.trim()) { avatar = img.src.trim(); break; }
                                }
                            }

                            // time (raw string only)
                            let createdClean = '';
                            const t1 = root.querySelector('.comment_time .timestamp');
                            if (t1 && t1.textContent) {
                                createdClean = t1.textContent.trim();
                            } else if (selTime) {
                                const t = root.querySelector(selTime) as HTMLElement | null;
                                createdClean = (t?.getAttribute('title') || t?.textContent || '' ).trim();
                            }
                            createdClean = createdClean.replace(/\s+/g, ' ').trim();
                            const cMatch = createdClean.match(/\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}(?::\d{2})?/);
                            if (cMatch) createdClean = cMatch[0];

                            // like
                            let like = 0;
                            if (selLike) {
                                const n = root.querySelector(selLike) as HTMLElement | null;
                                like = Number((n?.textContent || '0').replace(/\D/g, '')) || 0;
                            }
                            ...
                            if (selHtml) {
                                const c = root.querySelector(selHtml) as HTMLElement | null;
                                html = c?.innerHTML || null;
                                text = c?.textContent?.trim() || null;
                            }

                            // strip leading mentions for replies
                            const isReplyFlag = !!parentCid || depth > 0;
                            const normalize = (s: string | null) => s ? s.replace(/[\u200B-\u200D\uFEFF]/g, '').replace(/\u00A0/g, ' ').trim() : s;
                            if (isReplyFlag) {
                                const stripText = (s: string | null) => {
                                    if (!s) return s;
                                    let out = normalize(s)!;
                                    let prev: string;
                                    const reAt = /^\s*@([^\s@]+)\s*님(?:[ :,\-·/]+)?/u;
                                    const rePlain = /^\s*([가-힣A-Za-z0-9_.\-]{2,32})\s*님(?:[ :,\-·/]+)?/u;
                                    do {
                                        prev = out;
                                        out = out.replace(reAt, '').trimStart();
                                        out = out.replace(rePlain, '').trimStart();
                                    } while (out !== prev);
                                    return out.trim();
                                };
                                const stripHtml = (s: string | null) => {
                                    ...
                                    let out = normalize(s)!;
                                    let prev: string;
                                    const reStrong = /^\s*(?:<br\s*\/?>\s*)*<strong[^>]*>[\s\S]*?<\/strong>\s*님(?:[ :,\-·/]+)?/i;
                                    const reAtText = /^\s*(?:<br\s*\/?>\s*)*@([^\s@<]+)\s*님(?:[ :,\-·/]+)?/u;
                                    const rePlainText = /^\s*(?:<br\s*\/?>\s*)*([가-힣A-Za-z0-9_.\-]{2,32})\s*님(?:[ :,\-·/]+)?/u;
                                    do {
                                        prev = out;
                                        out = out.replace(reStrong, '').trimStart();
                                        out = out.replace(reAtText, '').trimStart();
                                        out = out.replace(rePlainText, '').trimStart();
                                    } while (out !== prev);
                                    return out.trim();
                                };
                                text = stripText(text);
                            ...
                            out.push({ cid, parentCid, depth, author, avatar, createdClean, like, html, text });
                        }
                        return out;
                    },
                    {
                ...
                type Raw = { cid: string; parentCid: string | null; depth: number; author: string | null; avatar: string | null; createdClean: string; like: number; html: string | null; text: string | null; };
                const locals: { item: Comment; cid: string; parentCid: string | null }[] = [];
                const flat: Comment[] = [];

                for (const r of raws as Raw[]) {
                    let depth = r.depth;
                    if (!r.parentCid) depth = 0; else if (depth === 0) depth = 1;
                    const isReplyFlag = !!r.parentCid || depth > 0;
                    ...
                    const item: Comment = {
                        ...
                        content: r.text,
                        timestamp: parseKstToUtcIso(r.createdClean, `COMMENT ${cfg.BOARD_NAME} ${request.loadedUrl} postId=${postId} cid=${r.cid}`),
                        isReply: isReplyFlag,
                        replies: [],
                        raw: null,
                        html: r.html,
                    ...
                    flat.push(item);
                    locals.push({ item, cid: r.cid, parentCid: r.parentCid });
                }

                const idByCid = new Map<string, string>();
                for (const lc of locals) idByCid.set(lc.cid, lc.item.id);
                ...
                for (const lc of locals) {
                    ...
                    if (lc.parentCid && idByCid.has(lc.parentCid)) {
                        parentId = idByCid.get(lc.parentCid)!;
                        const p = byId.get(parentId);
                        if (p) {
                            const normalized = (typeof p.depth === 'number' ? p.depth! : 0) + 1;
                            if (!Number.isNaN(normalized)) item.depth = normalized;
                        }
                    ...
                    stack.push({ id: item.id, depth: item.depth ?? 0 });
                }
...
function pickBestTwitterThumb(
    ...
    const byPriority = [/card_img\//i, /amplify_video_thumb\//i, /ext_tw_video_thumb\//i, /media\//i];
...
async function collectEmbeds(page: import('playwright').Page, cfg: BoardConfig, log?: { info: Function; debug?: Function; warning?: Function }): Promise<Embedded[]> {
    ...
    // Map key: tweetId  →  value: {thumbnail, text, isVideo}
    const twFrameMeta = new Map<string, { thumbnail?: string | null; text?: string | null; isVideo?: boolean }>();
    try {
        ...
        for (const f of frames) {
            ...
            try {
                ...
                // Minimal single wait for any useful content to appear (cap to ~1.2s)
                try { await f.waitForSelector('video[poster], [data-testid="tweetText"], img[src*="twimg.com"], [style*="background-image"]', { timeout: 1200 }); } catch { }
                ...
                // Collect candidate image URLs from inside the tweet frame
                const imgCandidates: string[] = await f.$$eval(
                    'img[src], img[srcset]',
                    (els) => {
                        const urls: string[] = [];
                ...
                // Background-image URLs (cards often use bg images)
                const bgCandidates: string[] = await f.$$eval(
                ...
                // Filter to twimg domains and prefer media/card_img
                const allImg = Array.from(new Set([...imgCandidates, ...bgCandidates]))
                    .filter(u => /(?:twimg\.com|pbs\.twimg\.com)/i.test(u));
                const thumb = pickBestTwitterThumb(allImg, poster, null);
                const isVideo = Boolean(poster) || allImg.some(u => /amplify_video_thumb|ext_tw_video_thumb/i.test(u));

                if (frameId && (thumb || text)) twFrameMeta.set(frameId, { thumbnail: thumb, text, isVideo });
    ...
    const twEmbeds = await Promise.all(
        [...tweetIds].map(async ([id, url]) => {
            ...
            // When tweet is image-only, set videoId to empty string. If we couldn't detect, keep id for safety.
            const videoIdField = (inFrame && inFrame.isVideo === false) ? '' : id;
            return {
                type: 'X',
                url,
                provider: 'X',
    ...
    const iframeSrcs = iframeSel
        ? await page.$$eval(iframeSel, (els) => (els as HTMLIFrameElement[]).map(el => el.getAttribute('src')))
            .catch(() => [] as (string | null)[])
        : [];
    const allIframeSrcs = normalizeAbsolute(base, iframeSrcs);
