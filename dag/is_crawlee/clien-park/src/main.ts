 ...
 *
 * 역할
 *  - 실행 진입점(entry). **셀렉터/규칙** 은 routes.ts에서 정의하며,
 *    본 파일은 실행 모드/CLI 파싱/결과 저장·검증을 담당합니다.
 *
 * 모드
 *  • incremental (기본): 최근 페이지만 조회, 최신 postId 저장
 *  • full            : --hours/--minutes 기준으로 cutoff 이전 글에서 중단
    ...
    switch (input.mode) {
        ...
        case 'hot': {
            ...
            startUrls = hotUrls.map((url: string) => {
                ...
                try {
                    const u = new URL(url);
                    const mm = u.pathname.match(/\/service\/board\/([^/]+)\/(\d+)/);
                    if (mm) {
                        boardName = mm[1];
                        postId = mm[2];
                    } else {
                        const idm = u.pathname.match(/\/(\d+)(?:\/)?$/);
                        postId = idm ? idm[1] : null;
                    }
                } catch { /* keep nulls */ }
                return { url, label: 'DETAIL', userData: { postId, boardName } };
            });
